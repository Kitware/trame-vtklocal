...
HARD_UPDATE = False  # toggle between force update or throttle [need to be true to trigger stale state]
THROTTLE_RATE = 20  # time per seconds
MAX_MEMORY_USAGE = (2_000_000_000,)  # 2 GB
# -----------------------------------------------------------------------------
print(
    "Volume size: "
    f"{2 * DIMENSION} x {2 * DIMENSION} x {DIMENSION} = "
    f"{2 * DIMENSION * 2 * DIMENSION * DIMENSION}"
)
# -----------------------------------------------------------------------------
...
class ContourApp(TrameApp):
    ...
    def _setup_vtk(self):
        ...
        source = vtkRTAnalyticSource(
            whole_extent=(-DIMENSION, DIMENSION, -DIMENSION, DIMENSION, 0, DIMENSION),
        )
    ...
    def _build_ui(self):
        ...
        with DivLayout(self.server) as self.ui:
            ...
            with html.Div(
                ...
                html.Div(
                    "{{ utils.fmt.bytes(mem_vtk) }} / {{ utils.fmt.bytes(mem_total) }}",
                    style="color: white;",
                )
