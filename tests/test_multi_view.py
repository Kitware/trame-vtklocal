...
async def test_multi_view(MultiViewApp, utils):
    ...
    being swallowed leaves it black) and verifies that the current single
    JSPI-capable VTK bundle keeps the second view interactive.
    """
    ...
    async with async_playwright() as p:
        ...
        # Drag the second canvas and verify that its client-side camera state
        # changes. A rendering-only screenshot would miss an inert second view
        # because both views can still paint their initial frame.
        await page.evaluate(
            """() => {
